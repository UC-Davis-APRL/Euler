...
#define IMU_UPDATE_RATE_HZ 200
#define GNSS_UPDATE_RATE_HZ 1
#define HEIGHT_UPDATE_RATE_HZ 20
#define FILTER_2_UPDATE_RATE_HZ 200
...
private:
    ...
    float GRAVITY = 9.81e3;            //  mm/s^2             Magnitude of gravity at rest. Determines units of velocity. [UNITS MUST MATCH ACCELERATION]
    float SD_ACC  = 1000 / GRAVITY;    //  mm/s^2 / g-force   Standard deviation of acceleration. Deviations from zero are suppressed.
    float SD_VEL  = 200  / GRAVITY;    //  mm/s   / g-force   Standard deviation of velocity. Deviations from target value are suppressed.
    float ALPHA   = 0.5;               //                     Gain of heading update - See example "output" for more information.
...
inline void Nav::updateFilter()
{
    ...
    /*
        Velocity filter
    */
    vec3_t accel2 = { accel.acceleration.x, accel.acceleration.y, accel.acceleration.z }; // g-unit
    vec3_t gyro2 = { gyro.gyro.x, gyro.gyro.y, gyro.gyro.z }; // radians/second
    vec3_t vel_t = {0,0,0};
    // printf("ax: %lf ay: %lf az: %lf gx: %lf gy: %lf gz: %lf \n", accel.acceleration.x, accel.acceleration.y, accel.acceleration.z, gyro.gyro.x, gyro.gyro.y, gyro.gyro.z);
    vel_t /= GRAVITY;
    fusion.update( gyro2, accel2, vel_t, SD_ACC, SD_VEL, ALPHA ); 
    vel = fusion.getVel() * GRAVITY;
